...
Utilizzando il Teorema Centrale del Limite si può avere una idea più chiara di cosa sia diventata la passeggiata aleatoria. \\
Sia  \(x_j = x(j\tau)\) la posizione raggiunta dopo \(j\) passi e posto \(j \geq 1\) 
\[
  h\xi_j = x_j - x_{j -1} 
\]
Le variabili \(\xi_j\) sono aleatorie i.i.d. con valore \(1 \mbox{ o } -1\)
\end{document}