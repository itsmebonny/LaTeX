...
\begin{flalign*}
  p(x, t+\tau) = p(x, t) + p_t(x,t)\tau +\sigma(\tau) \\
  p(x\pm h, t) = p(x,t)\pm p_x(x,t)h+\frac{1}{2}p_{xx}(x,t)h^2+\sigma{h^2}
\end{flalign*}
che all'interno della (2.54) restituiscono, dopo le opportune semplificazioni
{\everymath={\displaystyle}
\[
  \begin{array}{c}
   p_t\tau + \sigma(\tau) = \frac{1}{2}p_{xx}h^2 +\sigma(h^2) \\
   \Downarrow \mbox{ dividendo per }\tau \\
   p_t + \sigma(1) = \frac{1}{2}\frac{h^2}{\tau} p_{xx}+\sigma\left(\frac{h^2}{\tau}\right) 
  \end{array}
\]
}
...
\]
La proprietà di Markov si può esprimere come una condizione \(H\), legata al comportamento della particella prima dell'istante \(s\geq 0\), che, con un determinato moto Browniano \(Y(t) = B^x (t+s)\), stabilisce l'indipendenza del processo futuro \(B^x(t+s)\) da quello prima dell'istante \(s\). L'assenza di memoria è una caratteristica della passeggiata aleatoria. \\
In alternativa esiste la proprietà di Markov forte che sostituisce all'istante \(s\) un'istante di tempo \(\tau\) aleatorio che dipende dal comportamento della particella nell'intervallo \([0, \tau]\).
\begin{itemize}
  \item Valore atteso
\end{itemize}
...
\begin{flalign*}
  p(x,t+\tau) = p(x,t) = p_t(x,t)\tau + \sigma(\tau) \\
  p(x \pm h, t) = p(x,t) \pm p_x(h,t)h + p_{xx}(x,t)h^2 + \sigma(h^2)
\end{flalign*}
Con le opportune semplificazioni, come prima si ottiene
{\everymath={\displaystyle}
\[
  \begin{array}{c}
   p_t\tau + \sigma(\tau) = \frac{1}{2}p_{xx}h^2 + (q_0 -p_0)hp_x + \sigma(h^2) \\
   \Downarrow \mbox{ dividendo per }\tau \\
   p_t + \sigma(1) = \frac{1}{2}\frac{h^2}{\tau} p_{xx}+\frac{(q_0-p_0)h}{\tau}p_x+\sigma\left(\frac{h^2}{\tau}\right) 
  \end{array}
\]
}
...
\begin{flalign}
  p_0 = \frac{1}{2} - \frac{\beta}{2}h + \sigma(h) \\
  q_0 = \frac{1}{2} + \frac{\beta}{2}h + \sigma(h)
\end{flalign}
da cui deriva 
\begin{equation}
