...
\[
  u(\bm{x}, t) = v(r,t)
\]
dove \(r = \abs*{\bm{x} - \bm{x}_0}\) e \(\bm{x}_0 \in \mathbb{R}^n\) è un punto fisso. \(u(\bm{x}, t) = e^{i\omega t}v(r)\) rappresenta una onda stazionaria sferica mentre \(u(\bm{x}, t) = v(r - ct)\) è l'equazine di una onda progressiva i cui fronti d'onda appartengono alle sfere di equazione \(r - ct = \text{costante}\), con velocità \(\abs*{c}\).
\end{itemize}
...
\begin{equation}
  u_{tt} - c^2u_{xx} = f
\end{equation}
dove \(c^2(x,t) = \frac{\tau_0(t)}{\rho_0(x)}\). In caso la corda sia omogenea (come quella del violino), allora \(\rho_0\) è costante. Può essere costante anche \(\tau_0\) se la corda è perfettamente elastica, ossia la tensione orizzontale è quasi la stessa della corda a riposo. \\
