...
\begin{equation}
  \int_{\Omega} v\Delta u \, d\textbf{x} = \int_{\partial\Omega} v\partial_{\bm\nu} u \, d\sigma - \int_{\Omega} \nabla v \cdot \nabla u \, d\textbf{x} \label{Identità di Green}
\end{equation}
...
\end{equation}
e il massimo di $u$ in $\overline{Q}_{T- \epsilon}$ è assunto in un punto di $\partial_p Q_{T-\epsilon}$. Infatti, si immagini che ciò non sia vero. Allora esiste un punto ($\mathbf{x}_0, t_0$), $\mathbf{x}_0 \in \Omega$, $0 < t_0 \leq T - \epsilon$, tale che $u$($\mathbf{x}_0, t_0$) è il massimo di $u$ in $\overline{Q}_{T-\epsilon}$. \\
...
\begin{definition}
  La funzione 
  \[
     \Gamma_D(\mathbf{x}, t) = \frac{Q}{(4\pi Dt)^{\frac{n}{2}}}e^{-\frac{|\mathbf{x}|^2}{4Dt}} \qquad (\mathbf{x} \in \mathbb{R}^n, t>0)
  \]
  si chiama \textbf{soluzione fondamentale} dell'equazione di diffusione in dimensione \(n\).

\end{definition}
Si può generalizzare al caso multidimensionale anche la distribuzione di Dirac in un punto \(\mathbf{y}\) come
\[
\int\delta_n (\mathbf{x} - \mathbf{y}) \phi (\mathbf{x}) d\mathbf{x} = \phi(\mathbf{y})  
\]
con \(\phi\) funzione di test, continua su \(\mathbb{R}^n\) e nulla al di fuori di un insieme chiuso e limitato. \\
La soluzione fondamentale \(\Gamma_D(\mathbf{x} - \mathbf{y}, t)\) per un \(\mathbf{y}\) fissato è l'unica soluzione del problema di Cauchy
\[
  \begin{cases}
    u_t -D\Delta u = 0 & \mathbf{x}\in \mathbb{R}^n, \, t>0 \\
    u(\mathbf{x}, 0) = \delta_n (\mathbf{x} -\mathbf{y}) & \mathbf{x} \in \mathbb{R}^n
  \end{cases}
\]
...
\begin{enumerate}
  \item \emph{Convezione o trasporto}. Il flusso è determinato dalla corrente d'acqua, nel caso in cui l'inquinante mantenga la sua forma senza deformarsi; modellizzabile come
  \[
    q(x,t) = vc(x,t)
  \]
...
Questa sezione si occuperà di studiare il problema di Cauchy globale nel caso omogeneo unidimensionale.
...
\begin{proof}
  ...
  \[
    \int_{\Omega} \abs*{\nabla w}^2 \, d\textbf{x} = \int_{\partial\Omega} w\partial_{\bm{\nu}} w\, d\sigma
  \]
...
Anche con le funzioni armoniche esiste una stretta correlazione con la passeggiata aleatoria e il moto Browniano. Fissata una dimensione \(n = 2\) (si può estendere il ragionamento a ogni dimensione), siano \(\tau > 0\) il passo temporale e \(h > 0\) quello nello spazio. Sia a sua volta \(h\mathbb{Z}^2\) il reticolo di punti \(\textbf{x} = (x_1, x_2)\), con coordinate multipli interi di \(h\). Con \(p(\textbf{x}, t) =  p(x_1, x_2, t)\) che rappresenta la probabilità di trovare la particella in un punto \(\textbf{x}\) al tempo \(t\). L'equazione alle differenze per \(p\) in dimensione due si scrive come
\[
  p(\textbf{x}, t+ \tau) = \frac{1}{4} \{p(\textbf{x}+h\bm{e}_1, t) + p(\textbf{x}-h\bm{e}_1, t) + p(\textbf{x}+h\bm{e}_2, t) + p(\textbf{x}-h\bm{e}_2, t)\}
\]
Per riscrivere tutto ciò si può introdurre l'operatore media \(M_h\) che si applica su generiche funzioni \(u =  
u(\textbf{x})\) come
\begin{flalign*}
  M_hu(\textbf{x}) &= \frac{1}{4}\{u(\textbf{x}+h\bm{e}_1, t) + u(\textbf{x}-h\bm{e}_1, t) + u(\textbf{x}+h\bm{e}_2, t) + u(\textbf{x}-h\bm{e}_2, t)\} = \\
  & = \frac{1}{4}\sum_{\abs*{\textbf{x}- \textbf{y}} = h}u(\textbf{y})
\end{flalign*}
Si noti che \(M_hu(\textbf{x})\) è la media aritmetica dei valori di \(u\) nei punti del reticolo \(h\mathbb{Z}^2\), posti a distanza \(h\) da \(\textbf{x}\).
L'equazione alle differenze allora prende la forma
\begin{equation}
  p(\textbf{x}, t + \tau) = M_hp(\textbf{x}, t)
\end{equation}
...
\[
Mu(\textbf{x}) = u(\textbf{x}) + \frac{h^2}{4}\{u_{x_1x_1}(\textbf{x})+ u_{x_2x_2}(\textbf{x}) + o(h^2)\} 
\]} si ha
\begin{equation}
\lim_{h \to 0} \frac{M_h u(\textbf{x}) - u(\textbf{x})}{h^2} \to \frac{1}{4} \Delta u(\textbf{x})  
\end{equation}
...
Una funzione \emph{d-armonica} è una funzione che in ogni punto \(\textbf{x}\) coincide con la media dei valori nei punti in un intorno discreto di \(\textbf{x}\) di raggio \(h\).
Per definire un problema di Dirichlet discreto, sia \(A\) un sottoinsieme limitato di \(h\mathbb{Z}^2\) e \(\mathbf{x} \in A\):
\begin{itemize}
  \item \(\mathbf{x}\) è un punto interno di \(A\) se il suo intorno discreto di raggio \(h\) è contenuto in \(A\)
  \item \textbf{x} è un punto di frontiera se non è interno ma il suo intorno discreto contiene almeno un punto interno. L'insieme dei punti di frontiera si indica come \(\partial A\)
\end{itemize}
...
\begin{enumerate}
  \item Principio di massimo: se \(u\) assume il suo massimo (o minimo) in un punto interno allora \(u\) è costante. Si immagini che \(\textbf{x} \in A\) sia punto interno e che \(u(\textbf{x}) = M \geq u(\textbf{y})\) per ogni \(\textbf{y} \in A\). Poiché \(u(\textbf{x})\) è la media aritmetica dei valori assunti nei quattro punti posti a distanza \(h\) da \(\textbf{x}\), in ognuno di questi punti \(u\) deve avere lo stesso valore di \(M\). Allo stesso modo, sia 8\(\mathbf{x}_1 \not = \mathbf{x}\) uno dei quattro punti, \(u(\mathbf{y}) = M\) per ogni \(\mathbf{y}\) nell'intorno di raggio \(h\) di \(\mathbf{x}_1\). Il fatto che \(A\) sia un insieme connesso e finito, permette di mostrare che \(u(\textbf{y}) = M\) in ogni punto di \(A\). \\
  Per questo motivo il massimo (o minimo) di \(u\) è assunto sulla frontiera di \(A\).
  \item La soluzione del problema di Dirichlet è unica, essendo la soluzione \(u\) una funzione d-armonica, si immagini di avere una funzione \(v\) anche essa soluzione del problema di Dirichlet. Allora anche \(v\) deve essere una funzione d-armonica che rispetti il principio di massimo sopra enunciato. Ma poiché la frontiera di \(A\) è sempre la stessa \((u-v)(\mathbf{x} = 0 \, \forall \mathbf{x} \in A)\), quindi il massimo (o minimo) di \(v\) coincide con quello di \(u\), ossia le due funzioni coincidono.
\end{enumerate}
Dal problema (3.9) sorge una interessante interpretazione probabilistica, che è di grande aiuto per la costruzione di una soluzione. La funzione \(g\) della passeggiata aleatoria della particella rappresenta un \emph{guadagno}, ossia, con una particella che parte da \(\mathbf{x}\) e termina la sua passeggiata sulla frontiera di \(A\) in \(\mathbf{y}\), allora il guadagno è pari a \(g(\mathbf{y})\). \\
Si vuole mostrare che la particella, indipendentemente dal punto di partenza, raggiunge sempre la frontiera \(\partial A\), e che il valore \(u(\mathbf{x})\) rappresenta il valore atteso del guadagno \(g\) rispetto a una opportuna distribuzione di probabilità su \(\partial A\). Sia \(\Gamma \subseteq \partial A\), allora 
\[
  P(\mathbf{x}, \Gamma)
\]
indica la probabilità che la particella, partendo da \(\mathbf{x}\), giunga per la prima volta in un punto \(\mathbf{y} \in \Gamma\) sulla frontiera. Si vuole mostrare che \(P(\mathbf{x}, \partial A) = 1 \quad \forall \mathbf{x} \in A\).
Nel caso in cui \(\mathbf{x} \in \Gamma\), si ottiene \(P(\mathbf{x}, \Gamma) = 1\), mentre con \(\mathbf{x} \in \partial A\backslash \Gamma\) si ha \(P(\mathbf{x}, \Gamma) = 0\). \\
Fissato l'insieme \(\Gamma\) si ha che 
\[
  \mathbf{x} \longrightarrow w_{\Gamma}(\mathbf{x}) \equiv P(\mathbf{x}, \Gamma)
\]
...
\]     
la probabilità di transizione, ossia la probabilià di passare dal punto \(\mathbf{x} \mbox{ a } \mathbf{y}\) in un solo passo. La simmetria del moto permette di sapere che \(p(1, \mathbf{x}, \mathbf{y}) =  \frac{1}{4}\) se \(\abs*{\mathbf{x} -\mathbf{y}} = 1\), altrimenti \(p(1, \mathbf{x}, \mathbf{y}) = 0\). Poiché \(w_{\Gamma}(\mathbf{x})\) può essere ottenuta come somma di eventi disgiunti ed esaustivi (ossia la cui somma è l'evento certo), si ottiene così:
\[
  w_{\Gamma}(\mathbf{x}) = \sum_{\mathbf{y} \in h\mathbb{Z}^2}p(1, \mathbf{x}, \mathbf{y})w_{\Gamma}(\mathbf{y}) = M_hw_{\Gamma}(\mathbf{x})
\]
che è, di fatto, l'equivalente di 
\[
  (I - M_h)w_{\Gamma} = \Delta^*_h w_{\Gamma} = 0
\]
da cui si deduce che la funzione \(w_{\Gamma}\) è d-armonica in \(A\). Nel caso in cui \(\Gamma \equiv \partial A\), la funzione \(w_{\partial A}\) è sempre d-armonica e assume valore \(1\) al bordo. Inoltre, poiché la soluzione del problema di Dirichlet è unica, si ha \(w_{\partial A} \equiv 1\) in tutto \(A\). Pertanto raggiungere \(\partial A\) partendo da un punto \(\mathbf{x}\) è un evento certo, per cui si nota che la funzione 
\[
\Gamma \longrightarrow P(\mathbf{x}, \Gamma)  
\]
è additiva e definisce una misura di probabilità su \(\partial A\). \\
La soluzione \(u\) della (3.9) diventa facile da comporre:
\newpage
\begin{theorem}
  Il valore della soluzione del problema di Dirichlet discreto nel punto \(\mathbf{x}\) è dato dal valore atteso del guadagno \(g(\cdot)\) rispetto alla probabilità \(P(\mathbf{x}, \cdot)\), ossia 
  \begin{equation}
    u(\mathbf{x}) = \sum_{\mathbf{y}\in \partial A} g(\mathbf{y})P(\mathbf{x}, \{\mathbf{y}\})
  \end{equation}
\end{theorem}
\begin{proof}
  Ogni addendo
  ...
  Se \(\mathbf{x} \in \partial A\) allora \(u(\mathbf{x}) = g(\mathbf{x})\) dal momento che ogni termine della somma è pari a \(g(\mathbf{x})\) se \(\mathbf{x} = \mathbf{y}\) o è nullo se \(\mathbf{x} \not = \mathbf{y}\).
\end{proof}
Il caso discreto permette di stabilire alcune proprietà fondamentali delle funzioni armoniche. Una funzione è armonica in un dominio \(\Omega \subseteq \mathbb{R}^n\) se \(u \in \mathcal{C}^2(\Omega) \mbox{ e }\Delta u = 0 \mbox{ in } \Omega\). Dal momento che le funzioni d-armoniche hanno una definizione in termini di media, ci si può aspettare che anche le funzioni armoniche godano di proprietà simili. Si immagini una sfera \(B_r(\mathbf{x})\)\footnote{Con questa dicitura si intende una sfera di raggio \(r\) centrata nel punto \(\mathbf{x}\).}, che sia \(\subset \subset\)\footnote{Il simbolo \(\subset \subset\) indica una \emph{chiusura compatta contenuta}, ossia il sottoinsieme contenuto all'interno non ha punti di contatto con l'insieme principale.}. Nel caso di una sfera \(B_r \subset \subset \Omega\) il valore al centro della sfera uguaglierà la media aritmetica dei valori al bordo \(\partial B_r\). Sapendo che \(\omega_n\) indica la misura superficiale di \(\partial B_1\) e \(\abs*{B_1} = \frac{\omega_n}{n}\) rappresenta il volume.
\begin{theorem}
  Sia \(u\) funzione armonica in \(\Omega \subset \mathbb{R}^n\). Per ogni sfera \(B_r(\mathbf{x}) \subset \subset \Omega\), valgono le formule
  \begin{eqnarray}
    u(\mathbf{x}) = \frac{n}{\omega_n R^n} \int_{B_R(\mathbf{x})u(\mathbf{y})} \, d\mathbf{y} \\
    u(\mathbf{x}) = \frac{1}{\omega_n R^{n-1}} \int_{\partial B_R(\mathbf{x})} u(\bm{\sigma}) \, d\sigma
  \end{eqnarray}
  con \(d\sigma\) che rappresenta l'elemento di superficie su \(\partial B_R(\mathbf{x})\).
\end{theorem}
\begin{proof}
  La seconda formula si può scrivere, ponendo \(r < R\)
  \[
    g(r) = \frac{1}{\omega_n R^{n-1}}\int_{\partial B_r(\mathbf{x})} u(\bm{\sigma}) \, d\sigma
  \]
  Attuando un cambio di variabili e ponendo \(\bm{\sigma} = \mathbf{x} + r\bm{\sigma}'\) si avranno come estremi di integrazione \(\bm{\sigma}' \in \partial B_1(\bm{0}), \, d\sigma = r^{n-1}d\sigma'\) e quindi
  \[
    g(r) = \frac{1}{\omega_n} \int_{\partial B_1(\bm{\sigma})} u(\mathbf{x}+r\bm{\sigma}') \, d\sigma'
  \] 
  Posta una funzione \(v(\mathbf{y})\) in modo che \(v(\mathbf{y}) = u(\mathbf{x} + r\mathbf{y})\). 
  \newpage
  Questo permette di ottenere: 

  \begin{eqnarray*}
    \nabla v(\mathbf{y}) = r\nabla u(\mathbf{x} + r\mathbf{y}) \\
  \Delta v(\mathbf{y}) = r^2\Delta u (\mathbf{x} + r\mathbf{y})
  \end{eqnarray*}
  Si ha, allora, 
  \begin{eqnarray*}
  g'(r) & = & \frac{1}{\omega_n} \int_{\partial B_1(\bm(0))} \frac{d}{dr}u(\mathbf{x} + r\bm{\sigma}') \, d\sigma' = \frac{1}{\omega_n} \int_{\partial B_1(\bm{0})} \nabla u(\mathbf{x} + r\sigma') \cdot \bm{\sigma}' \, d\sigma' \\
  & = & \frac{1}{\omega_n r} \int_{\partial B_1 (\bm{0})} \nabla v(\bm{\sigma}') \cdot \bm{\sigma}' \, d\sigma' = (\mbox{Teorema della Divergenza}) \\
  & = & \frac{1}{\omega_n r} \int_{B_1 (\bm{0})} \Delta v(\mathbf{y}) \, d\mathbf{y} = \frac{r}{\omega_n} \int_{B_1(\bm{0})} \Delta u(\mathbf{x} + r\mathbf{y}) \, d\mathbf{y} = 0
  \end{eqnarray*}
  Per questo motivo \(g\) è costante, poiché \(g(r) \to u(\mathbf{x})\) con \(r \to 0\) si ottiene la (3.12). \\
  La (3.11) si ottiene moltiplicando la (3.12) per \(r^{n-1}\) e integrando entrmabi i membri tra \(0 \mbox{ e } R\) si ottiene
  \[
    \frac{R^n}{n} u(\mathbf{x}) = \frac{1}{\omega_n} \int_0^Rdr \int_{\partial B_r(\mathbf{0})} u(\bm{\sigma}) \, d\sigma = \frac{1}{\omega_n} \int_{B_R(\mathbf{x}}) u(\mathbf{y}) \, d\mathbf{y}
  \]
  che restituisce la (3.11)
\end{proof}
L'inverso del Teorema (3.3) permette di affermare che una funzione continua \(u\) soddisfa la proprietà di media in \(\Omega\) se la (3.11) o la (3.12) valgono per ogni sfera \(B_R(\mathbf{x}) \subset \subset \Omega\).
...
\begin{theorem}[Principio di massimo (o minimo)]
  Se \(u \in \mathcal{C}(\Omega)\) ha la proprietà di media nel dominio \(\Omega \subseteq \mathbb{R}^n\) e \(\bm{p} \in \Omega\) è un punto di massimo (o minimo) globale per \(u\), allora \(u\) è costante. In particolare, nel caso \(\Omega\) sia limitato e \(u \in \mathcal{C}(\overline{\Omega})\) non sia costante, allora \(\forall \mathbf{x} \in \Omega\):
  \[
  u(\mathbf{x}) < \max_{\partial \Omega} u \qquad (u(\mathbf{x}) > \min_{\partial \Omega} u) 
  \]
\end{theorem}
\begin{proof}
  Sia \(u\) non costante e \(\bm{p}\) un punto di minimo:
  \[
    m = u(\bm{p}) \leq u(\mathbf{y}) \qquad \forall \mathbf{y} \in \Omega
  \]
  Si prenda un ulteriore punto \(\bm{q}\) all'interno di \(\Omega\). Per le proprietà di \(\Omega\)\footnote{Insieme aperto e connesso} si può determinare una sequenza finita di sfere \(B(\mathbf{x}_j) \subset \subset \Omega, j = 0, \ldots, N\) tali che 
  \begin{itemize}
    \item \(\mathbf{x}_j \in B(\mathbf{x_{j-1}})\) per ogni \(j = 1, \ldots, N\)
    \item \(x_0 = \mathbf{p}, x_N = \mathbf{q}\)
  \end{itemize}
  Per la proprietà di media (\(\abs*{B(\mathbf{p})}\) indica il volume della sfera \(B(\mathbf{p})\)):
  \[
    m = u(\bm{p}) = \frac{1}{\abs*{B(\bm{p})}} \int_{B(\bm{p})} u(\mathbf{y}) \, d\mathbf{y}
  \]
Se esiste \(\bm{z} \in B(\bm{p})\) tale che \(u(\bm{z}) > m\), allora, presa una sfera \(B_r(\bm{z}) \subset B(\bm{p})\), si può scrivere
\begin{eqnarray*}
  m & = & \frac{1}{\abs*{B(\bm{p})}} \int_{B(\bm{p})u(\mathbf{y})} \, d\mathbf{y} \\
  & = & \frac{1}{\abs*{B(\bm{p})}} \left\lbrace \int_{B(\bm{p})\backslash B_r(\bm{z})} u(\mathbf{y}) \, d\mathbf{y} + \int_{B_r(\bm{z})} u(\mathbf{y}) \, d\mathbf{y} \right\rbrace
\end{eqnarray*}
Grazie alla proprietà di media si ha
\begin{equation}
  \int_{B_r(\bm{z})} u(\mathbf{y}) \, d\mathbf{y} = u(\bm{z})\abs*{B_r(\bm{z})} > m\abs*{B_r(\bm{z})}
\end{equation}
