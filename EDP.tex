...
\begin{proof}
  ...
  \] 
  Posta una funzione \(v(\mathbf{y})\) in modo che \(v(\mathbf{y}) = u(\mathbf{x} + r\mathbf{y})\). Questo permette di ottenere
  \begin{eqnarray*}
...
\begin{proof}
  ...
  \]
  Si prenda un ulteriore punto \(\bm{q}\) all'interno di \(\Omega\). Per le proprietà di \(\Omega\)\footnote{Insieme aperto e connesso} si può determinare una sequenza finita di sfere \(B(\mathbf{x}_j) \subset \subset \Omega\)
\end{proof}
\end{document}