...
\begin{proof}
    ...
    \[
        \exists \{u_n\} \subset H \text{ s.t. } \norm{u_n}=1 \qquad \abs{<Tu_n, u_n>} \to d    
    \]
    ...
    \[
        \abs{ <T u_{n_k}, u_{n_k}> - <Tu, u> \to 0} \qquad \text{ as } K \to \infty    
    \]
    We know
    \[
        \abs{<Tu_{n_k}, u_{n_k}} \leq \abs{<T u_{n_k} - Tu, u_{n_k}>} + \abs{ <Tu, u_{n_k} - u>} \to 0
    \]
    and also that \(\abs{<T u_{n_k}, u_{n_k}>} \to d\)
    \[
        \Rightarrow \abs{<Tu, u>} = d
    \]
    and hence \(u \neq 0\)

    \begin{itemize}
        \item Suppose that \(<Tu, u> = d\). Then
        \[
            \norm{Tu- du}^2 = \norm{Tu}^2 - 2d <Tu, u> + d^2 \norm{u}^2 \leq d^2 - 2d^2 + d^2 = 0
        \]
        \[
            \Rightarrow \norm{Tu - du} = 0 \Rightarrow Tu = du
        \]
        and \(d\) is an eigenvalue.
        \item \(<Tu, u> = -d\). Then one can prove that \(-d\) is an eigenvalue.
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    \]
    \[
        \lambda_1 <u_1, u_2 > = <Tu_1, u_2> = <u_1, T u_2> = \lambda_2 <u_1, u_2>
    \]
    \[
        \Rightarrow (\lambda_1 - \lambda_2) <u_1, u_2> = 0 \Rightarrow <u_1, u_2> = 0
    \]
\end{proof}
...
\begin{proof}
    \begin{itemize}
        ...
        \]
        \item From proposition 1, \(\exists \) an eigenvalue \(\lambda\) with \(\abs{\lambda_0} = \norm{T}_{\mathcal{L}(H)}\). Let \(V_0\) be the associated eigenspace. By proposition \(2\), \(\dim V_0 = N_0 < \infty\).
        ...
        \[
            u \in H_1 \iff <u, w> = 0 \quad \forall\; w \in V_0
        \]
        \[
            <Tu, w> = <u, Tw> = <u, \lambda_0 w> = \lambda_0 <u, w>
        \]
        ...
        \[
            ...
            \abs{<Tu, u>}
        \]
        Clearly, \(\abs{\lambda_1} \leq \abs{\lambda_0} = \sup \abs{<Tu, u>}\).
        ...
        \]
        \(\norm{w_i}^2 + \norm{w_j}^2 -2<w_i, w_j>\) \(\Rightarrow \lambda_n \to 0\).
...
\begin{proof}
    ...
    and 
    \[
        y = \sum_{n=1}^\infty <y, u_n> u_n
    \]
    Then, the equation \(x - Tx = y\) becomes
    \[
        \sum_{n=1}^\infty (1- \lambda_n) <x, u_n> u_n = \sum_{n=1}^\infty <y, u_n > u_n
    \]
    \[
        \Rightarrow (1 - \lambda_n) <x, u_n> = <y, u_n> \quad \forall\; n
    \]
    If \(\lambda_n \neq 1\) \(\forall \; n\), then we take 
    \[
        <x, u_n> = \frac{<y, u_n >}{1 - \lambda_n} \quad \forall\; n
    \]
    \[
        x = \sum_{n=1}^\infty \frac{<y, u_n>}{1 - \lambda_n} u_n
    \]
    is the solution. If instead \(\lambda_n = 1\) for some \(n\), then there are no solution if \(y\) is such that \(<y, u_n> \neq 0\):
    \[
        (1 - \lambda_n) <x, u_n> = <y, u_n>
    \]
    For \(y\) s.t. \(<y, u_n> = 0\), we have \(\infty\) many solutions.
\end{proof}