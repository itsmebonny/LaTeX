...
\begin{itemize}
    ...
    \end{array}
    \Longrightarrow v \in \mbox{BV}([0,1]) 
    \]
    But \(v \not \in \mbox{AC}([0,1])\)
...
\underline{Preliminary result}: \(A \in \real\) open. Then 
\[
...
\underline{Preliminary result}: \(f \in \mbox{BV}([a,b])\). Then
\[
...
\begin{proof}
    ...
    \]
\end{proof}
\underline{Preliminary result}: If \(G \in \mbox{AC}([a,b])\), then \(G \in \mbox{BV}([a,b])\), and moreover 
\[
