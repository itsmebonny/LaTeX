...
We learned that \(\nu: \mathbb{M} \to \left[0, \infty \right]\) by \[\nu(E)= \int_E \oldphi \, d\mu\] is a measure on \(X, \mathbb{M}\).
...
\begin{theorem}[Radon Nykodim Theorem]
    ...
    \(\oldphi = \frac{d \nu}{d \mu}\) namely \(\nu(E)= \int_E \oldphi \, d\mu \; \forall E \in \mathbb{M}\)
\end{theorem}

\begin{remark}
    if \(\mu\) is not sigma finite the theorem may fail.
    In \(\left(\left[0, 1\right], \mathbb{L}\left(\left[0, 1\right]\right)\right)\) consider the counting measure \(\mu = \mu_c\) and the lebesque measure \(\nu= \lambda\)
...
Note that \(\mu_c (\left[0, 1\right]) = \infty \) and \(\left( \left[0,1\right], \mathbb{L}(\left[0, 1\right]), \mu_c\right)\) is not \(\sigma\) - finite (\(\left[0,1\right]\) is uncountable)
...
\begin{definition}
    ...
    \[  R = \{ A \times B \subset X\times Y \text{ s.t. } A \in \mathbb{M}, b \in \mathbb{N}\}\]
    We define the product \(\sigma\) algebra \(\mathbb{M} \otimes \mathbb{N}\) as \(\sigma_0(R)\). \\
...
\begin{proposition}
    \(\left( X, \mathcal{M} \right), \left( Y, \mathcal{N} \right)\) measurable spaces. \(E \in \mathbb{M} \otimes \mathbb{N}\) \\
    Then \(E_x \in \mathbb{N} \) and \(E_y \in \mathbb{N} \) 
