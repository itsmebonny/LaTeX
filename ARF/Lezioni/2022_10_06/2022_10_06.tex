...
\(\Rightarrow \) by theorem 2, \(\chi_{T_\epsilon}\) is not \(R(I)\)
...
\begin{proposition}
    \((X, \mathcal{M}, \mu)\) complete measure space. Let \(\{f_n\} \subseteq \mathcal{L}'(X, \mathcal{M}, \mu)\). Suppose that \(\sum_{n=1}^\infty \int_X |f_n| \, d\mu < \infty\)
...
It is immediate to check that \(d_1 (f, g) = d_1(g, f)\) (symmetry)  

\(d_1(f, g) \leq d_1(f, h) + d_1(h, g) \; \forall f, g, h \in \mathcal{L}'(X)\) (triangular inequality)  
