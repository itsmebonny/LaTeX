...
\begin{itemize}
  ...
  \item \(A\) quasi certo: \(A \ind B \quad \forall B \in \mathcal{A}.\) Infatti \(P(B) = \mathcal{P}(B, A) + \mathbb{P}(B, A^C)\), ma \(\mathbb{P}(B, A^C) = \mathbb{P}(A^C) = 0\) 
\end{itemize}
...
\]
In termini probabilistici significa che: \(\Omega: \mbox{ mazzo di carte}, \; \#\Omega = 52, \; \mathcal{A} = 2^{\Omega}\). Se \(\mathbb{P}\) è uniforme su \(\mathcal{A}\), allora \(p_{\omega} = \frac{1}{52} \; \forall \omega \in \Omega\). Per verificare l'indipendenza
\end{document}