...
\begin{itemize}
    ...
    \item \textbf{Ruolo nella Ricerca:}
        \begin{itemize}
              ...
              \end{itemize}
                        % --- Esempio Box Inserito Qui ---
                        \begin{example} % Usa l'ambiente personalizzato
                            Supponiamo di voler studiare se il consumo di \textit{caffeina} migliora il \textit{tempo di reazione} in un compito al computer.
                            \begin{itemize}[leftmargin=*, label=-]
                                \item \textbf{VI (Causa Manipolata):} Quantità di Caffeina somministrata. Il ricercatore crea due o più livelli (gruppi): es. Gruppo A (0mg - placebo), Gruppo B (100mg).
                                \item \textbf{VD (Effetto Misurato):} Tempo di Reazione (in millisecondi) al compito. Si misura questa variabile per vedere se cambia in base al gruppo (livello di caffeina).
                                \item \textbf{Possibile Confondente Operazionale:} Se il Gruppo B riceve la caffeina in un caffè zuccherato e il Gruppo A solo acqua, non sappiamo se l'eventuale miglioramento è dovuto alla caffeina o allo zucchero (o entrambi). Lo zucchero è \textit{confuso} con la caffeina nell'operazionalizzazione della VI.
                                \item \textbf{Possibile Confondente Procedurale (di Disturbo):} Se testiamo il Gruppo B al mattino (quando sono più freschi) e il Gruppo A al pomeriggio (quando sono più stanchi), l'ora del giorno (variabile estranea) \textit{covaria} con la VI (caffeina) e potrebbe influenzare la VD (tempo di reazione), indipendentemente dalla caffeina. Bisognerebbe testare tutti alla stessa ora o bilanciare le ore tra i gruppi.
                            \end{itemize}
                            \end{example}
                            % --- Fine Esempio Box ---
